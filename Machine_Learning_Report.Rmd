...
I have loaded, two csv files contatining the training and test data was downloaded from Amazon's cloudfront on the 28/12/2015 into a data folder in the working directory. 
...
The training data was then loaded into R Studio.
...
I have found, there was a lot of NA values in the data which would create a lot of noise for the model. As a result, these columns were removed from the data set. The first eight columns that acted as identifiers for the experiment were also removed.
...
### Predictions
A separate data set was then loaded into R Studio nd cleaned in the same manner as before. The model was then used to predict the classifications of the 20 results of this new data, and write those predictions to individual files
...
preds <- as.character(predictTest)
